function [Offspring,outIndexList,chosengroups] = GLP_OperatorGAhalf(Problem,Parent1,Parent2,numberOfGroups,paramCtrl)
...
%--------------------------------------------------------------------------
    ...
    %% Parameter setting
    [proC,disC,disM] = deal(1,20,20);
    if nargin >= 5 && ~isempty(paramCtrl)
        if isscalar(paramCtrl)
            % Backward-compatible mode: one coefficient scales all parameters.
            coef = max(paramCtrl,0);
            proC = proC*coef;
            disC = disC*coef;
            disM = disM*coef;
        elseif numel(paramCtrl) == 3
            % Preferred mode: explicit [proC, disC, disM].
            proC = min(max(paramCtrl(1),0.6),1.0);
            disC = min(max(paramCtrl(2),2),40);
            disM = min(max(paramCtrl(3),5),60);
        end
    end
    [N,D] = size(Parent1);
