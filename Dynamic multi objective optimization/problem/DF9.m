...
% CEC2018 benchmark dynamic multiobjective optimisation problem DF9.
%
% Reference:
...
%   Optimisation", 2018.
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec,adds)
            ...
            sumv = zeros(size(PopDec,1),1);
            for i = 2 : obj.D
                target = cos(4*t + x1 + PopDec(:,i-1));
                sumv   = sumv + (PopDec(:,i) - target).^2;
            end
            g = 1 + sumv;
            amp = (1./(2*Nt) + 0.1);
            bump = max(0, amp .* sin(2*Nt*pi.*x1));
            f1 = g.*(x1 + bump);
            f2 = g.*(1 - x1 + bump);
            PopObj = [f1,f2];
        end
    ...
    methods(Access = private)
        function R = OptimumAt(~,N,t)
            ...
            u  = linspace(0,1,N-1)';           % distribute across segments
            seg = floor(u*Nt) + 1;
            seg(seg>Nt) = Nt;
            u2  = u*Nt - (seg-1);
            a   = (2*seg - 1)./(2*Nt);
