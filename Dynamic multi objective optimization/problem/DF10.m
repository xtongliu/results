...
% CEC2018 benchmark dynamic multiobjective optimisation problem DF10.
%
% Reference:
...
%   Optimisation", 2018.
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec,adds)
            ...
            x1 = PopDec(:,1);
            x2 = PopDec(:,2);
            core = sin(2*pi*(x1 + x2))./(1 + abs(Gt));
            g  = 1 + sum((PopDec(:,3:end) - core).^2,2);
            s1 = abs(sin(0.5*pi*x1));
            c1 = abs(cos(0.5*pi*x1));
            s2 = abs(sin(0.5*pi*x2));
            c2 = abs(cos(0.5*pi*x2));
            f1 = g.*(s1.^Ht);
            f2 = g.*((s2.*c1).^Ht);
            f3 = g.*((c2.*c1).^Ht);
            PopObj = [f1,f2,f3];
