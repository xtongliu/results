...
% CEC2018 benchmark dynamic multiobjective optimisation problem DF14.
%
% Reference:
...
%   Optimisation", 2018.
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec,adds)
            ...
            py = y1 + 0.05*sin(6*pi*y1);
            qy = 1 - y1 + 0.05*sin(6*pi*y1);
            px = x2 + 0.05*sin(6*pi*x2);
            qx = 1 - x2 + 0.05*sin(6*pi*x2);
            f1 = g.*qy;
            f2 = g.*qx.*py;
            f3 = g.*px.*py;
            PopObj = [f1,f2,f3];
